$( document ).ready( function () {
	...
	}, "You must write an observation.");

	$( "#form" ).validate( {
		rules: {
			status: 		{ required: true },
            observation:	{ fieldOperated: true }
		},
		errorElement: "em",
	...
	$("#btnSubmit").click(function(){
		...
		if ($("#form").valid() == true){
				...
				$.ajax({
					type: "POST",
					url: base_url + "external/update_vacation_status",
					data: $("#form").serialize(),
					dataType: "json",
					cache: false,
					...
					success: function(data){
						...
						if (data.status === "success") {
							window.location.href = base_url + "external/aprove_vacation/" + data.return;
						} else {
