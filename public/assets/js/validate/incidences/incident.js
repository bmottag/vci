$(document).ready(function () {
	$("#form").validate({
		rules: {
			...
			hour: { required: true },
			min: { required: true },
			callManager: { required: true },
			cause: { required: true },
			uderlyingCauses: { required: true },
			instructionBefore: { required: true },
	...
	$("#btnClose").click(function () {
		if (
			window.confirm("Are you sure you want to close this Near Miss report?")
		) {
			$.ajax({
				type: "POST",
				...
				success: function (data) {
					...
					if (data.result) {
						...
						var url = base_url + "incidences/incident";
	...
	$("#btnSubmit").click(function () {
		if ($("#form").valid() == true){
				...
				$.ajax({
					type: "POST",	
					url: base_url + "incidences/save_incident",	
					data: $("#form").serialize(),
					dataType: "json",
					cache: false,
					...
					success: function(data){
						...
						if (data.status === "success") {
							window.location.href = base_url + "/incidences/add_incident/" + data.idJob + "/" + data.idRecord;
						} else {
							alert('Error. Reload the web page.');
							$("#div_error").show();
						}
